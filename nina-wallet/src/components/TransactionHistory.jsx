...
function formatAmount(tx) {
  try {
    ...
    if (
      ...
      tx.rawContract?.decimals != null
    ) {
      const formatted = ethers.utils.formatUnits(
        tx.rawContract.value.toString(),
        tx.rawContract.decimals
      );
      const symbol = tx.asset || tx.rawContract.tokenSymbol || "TOKEN";
...
export default function TransactionHistory({ address }) {
  ...
  async function load() {
    ...
    try {
      ...
      console.log("Transfers returned:", transfers);

      const arr = transfers.map((t) => ({
        ...t,
        timestamp: t.metadata?.blockTimestamp
          ? new Date(t.metadata.blockTimestamp).toLocaleString()
          : "—",
      }));
    ...
    <div className="bg-white/5 p-6 rounded-2xl shadow-sm w-full max-w-4xl mx-auto">
      ...
      {!address ? (
        <div className="text-gray-400">
          Connect or create a wallet to see transactions.
        </div>
      ) : loading ? (
        ...
        <div className="text-gray-500">
          No transactions found on Sepolia for {shorten(address)}
        </div>
      ) : (
              ...
              {txs.map((tx) => {
                ...
                return (
                  <tr key={tx.hash} className="border-t">
                    ...
                    <td
                      className={`py-2 px-3 font-medium ${type === "IN" ? "text-emerald-600" : "text-rose-600"}`}
                    >
                      {type}
                    </td>
